...
public class CompareImages {
	...
	public int compareFeatcure(String filename1, String filename2) {
		int retVal = 0;
		long startTime = System.currentTimeMillis();
		...
		// Definition of ORB key point detector and descriptor extractors
		Feature2D feature = FastFeatureDetector.create();
		feature.detect(img1, keypoint1);
		feature.detect(img2, keypoint2);
		
		feature.compute(img1, keypoint1, descriptors1);
		feature.compute(img2, keypoint2, descriptors2);
		
//		feature.detectAndCompute(img1, img1, keypoint1, descriptors1);
//		feature.detectAndCompute(img2, img2, keypoint2, descriptors2);
...
//		extractor.compute(img2, keypoint2, descriptors2);
		...
		if( descriptors2.cols() == descriptors1.cols()) {
			...
			// Extract good image (distance are under 10)
			for (int i = 0; i < descriptors1.rows(); i++) {
				if(match[i].distance <= 10) {
					retVal++;
				}
